// Tool_Animation.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
                     ...
                     _In_ int       nCmdShow)
{
    ...
    unique_ptr<CMainAnimTool> pMainAnimTool = make_unique<CMainAnimTool>();
    if (nullptr == pMainAnimTool)
        return FALSE;

    pMainAnimTool->Initialize();
    ...
    while (true)
    {
        ...
        if (fTimeAcc > 1.f / 60.0f) {
            pMainAnimTool->Tick(CGameInstance::GetInstance()->ComputeTimeDelta(TEXT("Timer_60")));
            pMainAnimTool->Render();
