...
CUIInstanceDialogue::CUIInstanceDialogue()
{
...
HRESULT CUIInstanceDialogue::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUIInstanceDialogue::PriorityTick(_float _fTimeDelta)
{

}

void CUIInstanceDialogue::Tick(_float _fTimeDelta)
{
    KeyInput();
...
void CUIInstanceDialogue::LateTick(_float _fTimeDelta)
{
...
HRESULT CUIInstanceDialogue::Render()
{
    ...
    if (FAILED(GAMEINSTANCE->RenderFont(m_strNPCFont, m_strNPCTag, _float2((m_vUIPos[6].x - (m_vUISize[6].x * 0.5f)) + (g_iWinSizeX * 0.5f), (-m_vUIPos[6].y - (m_vUISize[6].y * 0.5f)) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_CUSTOMCENTER, _float4(0.79f, 0.65f, 0.28f, 1.f))))
        return E_FAIL;

    if (FAILED(GAMEINSTANCE->RenderFont(m_strContentFont, m_strContent, _float2((m_vUIPos[7].x - (m_vUISize[7].x * 0.5f)) + (g_iWinSizeX * 0.5f), (-m_vUIPos[7].y - (m_vUISize[7].y * 0.5f)) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_CUSTOMCENTER, _float4(1.f, 1.f, 1.f, 1.f))))
        return E_FAIL;

    if (FAILED(GAMEINSTANCE->RenderFont(m_strInteractionFont, m_strInteractionTag, _float2((m_vUIPos[8].x - (m_vUISize[8].x * 0.5f)) + (g_iWinSizeX * 0.5f), (-m_vUIPos[8].y - (m_vUISize[8].y * 0.5f)) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_END, _float4(0.65f, 0.52f, 0.42f, 1.f))))
        return E_FAIL;

    return S_OK;
}

void CUIInstanceDialogue::UpdateDialogue(const wstring& _strNPC, const wstring& _strContent, const wstring& _strInteraction, const string& _strNPCFont, const string& _strContentFont, const string& _strInteractionFont, _float4 _vNPCColor, _float4 _vContentColor, _float4 _vInteractionColor)
{
    m_strNPCTag = _strNPC;
    m_strContent = _strContent;
    m_strInteractionTag = _strInteraction;

    m_strNPCFont = _strNPCFont;
    m_strContentFont = _strContentFont;
    m_strInteractionFont = _strInteractionFont;

    m_vNPCColor = _vNPCColor;
    m_vContentColor = _vContentColor;
    m_vInteractionColor = _vInteractionColor;
}

void CUIInstanceDialogue::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    if (iNumMouse == 2 || iNumMouse == 3)
    {
        vertices[2].iSlotUV = 0;
...
HRESULT CUIInstanceDialogue::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIInstanceDialogue> CUIInstanceDialogue::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUIInstanceDialogue> pInstance = make_shared<CUIInstanceDialogue>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIInstanceDialogue::Create", MB_OK);
