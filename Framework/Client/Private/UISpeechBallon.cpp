...
CUISpeechBallon::CUISpeechBallon()
{
}
...
HRESULT CUISpeechBallon::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUISpeechBallon::PriorityTick(_float _fTimeDelta)
{

}

void CUISpeechBallon::Tick(_float _fTimeDelta)
{
    if (!m_isLoadingDone)
        return;
...
void CUISpeechBallon::LateTick(_float _fTimeDelta)
{
    if (!m_isLoadingDone)
        return;
...
HRESULT CUISpeechBallon::Render()
{
...
void CUISpeechBallon::UpdateSpeechBallon(wstring _strNPCName, wstring _strContent)
{
    m_strNPCName = _strNPCName;
    m_strContent = _strContent;

    if (m_strContent.size() > 20)
    {
        wchar_t ch = L'\n';
        m_strContent = m_strContent.insert(20, 1, ch);

        if (m_strContent.size() > 40)
            m_strContent.erase(40);
    }
}

HRESULT CUISpeechBallon::RenderFont()
{
    _float fNameOffset = static_cast<_float>(m_strNPCName.size()) * 7.f;
    _float fContentOffset = static_cast<_float>(m_strContent.size()) * 4.5f;

    if (FAILED(GAMEINSTANCE->RenderFont("Font_SuiteL12", m_strNPCName, _float2((m_vUIPos[0].x - fNameOffset) + (g_iWinSizeX * 0.5f), (-m_vUIPos[0].y - 10.f) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_END, _float4(1.f, 1.f, 1.f, 1.f))))
        return E_FAIL;

    if (FAILED(GAMEINSTANCE->RenderFont("Font_SuiteL10", m_strContent, _float2((m_vUIPos[3].x - fContentOffset) + (g_iWinSizeX * 0.5f), (-m_vUIPos[3].y - 20.f) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_END, _float4(1.f, 1.f, 1.f, 1.f))))
        return E_FAIL;

	return S_OK;
}

void CUISpeechBallon::InitUpdateTexture()
{
...
void CUISpeechBallon::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
...
HRESULT CUISpeechBallon::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUISpeechBallon> CUISpeechBallon::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUISpeechBallon> pInstance = make_shared<CUISpeechBallon>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUISpeechBallon::Create", MB_OK);
