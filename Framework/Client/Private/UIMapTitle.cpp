...
HRESULT CUIMapTitle::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
HRESULT CUIMapTitle::RenderFont()
{
    if (m_bIsScriptEnabled)
    {
        _float fContentOffset = static_cast<_float>(m_strDisplayContent.size()) * 14.5f;

        if (FAILED(GAMEINSTANCE->RenderFont("Font_DNFB24", m_strDisplayContent, _float2((m_vUIPos[0].x - fContentOffset) + (g_iWinSizeX * 0.5f), (-m_vUIPos[0].y - 20.f) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_END, _float4(0.79f, 0.65f, 0.28f, 0.2f), 0.f, _float2(0.f, 0.f), 1.f)))
            return E_FAIL;
    }
...
void CUIMapTitle::SetDisplayContent(const wstring& _strDisplayContent)
{
    m_fTimeAcc = 0.f;
    m_fAlphaTime = 0.f;
    m_fAlpha0Time = 0.f;
    m_bIsDone = false;
    m_strDisplayContent = _strDisplayContent;
}

void CUIMapTitle::InitUpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    vertices[0].vColor = _float4(1.f, 1.f, 1.f, 0.f);
    vertices[0].iSlotUV = 0;

    GAMEINSTANCE->GetDeviceContextInfo()->Unmap(instanceBuffer.Get(), 0);
}

void CUIMapTitle::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    m_fAlphaTime += m_fDeltaTime;

    _float fAlpha = LinearIp(0.f, 0.6f, m_fAlphaTime);
    if (fAlpha >= 0.5f)
        m_bIsScriptEnabled = true;

    vertices[0].vColor = _float4(1.f, 1.f, 1.f, fAlpha);

    GAMEINSTANCE->GetDeviceContextInfo()->Unmap(instanceBuffer.Get(), 0);
}

void CUIMapTitle::UpdateAfter()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    m_fAlpha0Time += m_fDeltaTime;

    _float fAlpha = LinearIp(0.6f, 0.f, m_fAlpha0Time);
    if (fAlpha <= 0.1f)
        m_bIsDone = true;
...
HRESULT CUIMapTitle::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIMapTitle> CUIMapTitle::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIMapTitle::Create", MB_OK);
