...
CUIInstanceWorldHP::CUIInstanceWorldHP()
{
...
HRESULT CUIInstanceWorldHP::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUIInstanceWorldHP::PriorityTick(_float _fTimeDelta)
{
  

}

void CUIInstanceWorldHP::Tick(_float _fTimeDelta)
{
    if (!m_bInitialize) {
        {
            ...
            _uint iNumTexs = static_cast<_uint>(m_vUIPos.size());

            for (_uint i = 0; i < iNumTexs; ++i)
            {
...
void CUIInstanceWorldHP::LateTick(_float _fTimeDelta)
{
...
HRESULT CUIInstanceWorldHP::Render()
{
...
void CUIInstanceWorldHP::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    vertices[6].vUVRatio = _float2(m_fRatio, 0.f);
    vertices[6].iSlotUV = 1;

    GAMEINSTANCE->GetDeviceContextInfo()->Unmap(instanceBuffer.Get(), 0);
}

HRESULT CUIInstanceWorldHP::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIInstanceWorldHP> CUIInstanceWorldHP::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUIInstanceWorldHP> pInstance = make_shared<CUIInstanceWorldHP>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIInstanceWorldHP::Create", MB_OK);
