...
CUIInstanceItemIcon::CUIInstanceItemIcon()
{
...
HRESULT CUIInstanceItemIcon::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUIInstanceItemIcon::PriorityTick(_float _fTimeDelta)
{
    
}

void CUIInstanceItemIcon::Tick(_float _fTimeDelta)
{
    KeyInput();
...
void CUIInstanceItemIcon::LateTick(_float _fTimeDelta)
{
    ...
    if (!m_pUIChildren.empty())
    {
        for (auto& childPair : m_pUIChildren)
        {
            if (childPair.second.lock() != nullptr)
            {
                if (childPair.second.lock()->GetUIType() == UI_INSTANCEITEMHOVER)
                {
                    if (childPair.second.lock()->GetUITag() == "InvEquipSlot0Hover")
...
HRESULT CUIInstanceItemIcon::Render()
{
...
void CUIInstanceItemIcon::IsRender(_float _fVisibleYTop, _float _fVisibleYBottom)
{
    ...
    vector<shared_ptr<CItemBase>>* pItemSlots = ITEMMGR->GetItemSlots(ITEM_EQUIP);
...
HRESULT CUIInstanceItemIcon::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIInstanceItemIcon> CUIInstanceItemIcon::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUIInstanceItemIcon> pInstance = make_shared<CUIInstanceItemIcon>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIInstanceItemIcon::Create", MB_OK);
