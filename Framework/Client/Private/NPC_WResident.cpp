...
CNPC_WResident::CNPC_WResident()
    : CNPC()
{
}

HRESULT CNPC_WResident::Initialize(_float3 _vCreatePos)
{
    // 설정된 위치에 생성
    ...
    m_pModel = GAMEINSTANCE->GetModel("WResident");
    m_strModelName = "WResident";
    AddComponent(L"Com_Model", m_pModel);
    m_pTransformCom->SetScaling(1.f, 1.f, 1.f);

    switch (m_iGenType)
    {
    case WRESIDENT_G_WALK:
        m_iActionState = WRESIDENT_A_IDLE1;
        m_pTransformCom->SetSpeed(0.7f);
        break;
    case WRESIDENT_G_IDLE1:
        m_iActionState = WRESIDENT_A_IDLE1;
        break;
    case WRESIDENT_G_NIGHTSEE:
        m_iActionState = WRESIDENT_A_NIGHTSEE;
        break;
    case WRESIDENT_G_IDLE2:
        m_iActionState = WRESIDENT_A_IDLE2;
        break;
    case WRESIDENT_G_SELL1:
        m_iActionState = WRESIDENT_A_SELL1;
        break;
    case WRESIDENT_G_SELL2:
        m_iActionState = WRESIDENT_A_SELL2;
        break;
    }
...
void CNPC_WResident::PriorityTick(_float _fTimeDelta)
{
    __super::PriorityTick(_fTimeDelta);
}

void CNPC_WResident::Tick(_float _fTimeDelta)
{
...
void CNPC_WResident::LateTick(_float _fTimeDelta)
{
...
shared_ptr<CNPC_WResident> CNPC_WResident::Create(_float3 _vCreatePos, _float3 _vInitLookPos, _int _iGenType, _float3 _vHairColor, _float3 _vHairSubColor)
{
    shared_ptr<CNPC_WResident> pInstance = make_shared<CNPC_WResident>();
    ...
    if (FAILED(pInstance->Initialize(_vCreatePos)))
        MSG_BOX("Failed to Create : CNPC_WResident");

    return pInstance;
}

HRESULT CNPC_WResident::AddRigidBody(string _strModelKey)
{
...
//UINT			eTargetColFlag = COL_NONE; /* 충돌할 객체의 Flag를 추가 ex) COL_MONSTER | COL_ITEM  */
    ...
    CapsuleDesc.strShapeTag = "NPC_WResident_Body";
...
void CNPC_WResident::OnContactFound(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

void CNPC_WResident::OnContactPersist(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

void CNPC_WResident::OnContactLost(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

//HRESULT CNPC_WResident::AddPartObjects(string _strModelKey)
//{
...
void CNPC_WResident::FSM_Seller(_float _fTimeDelta)
{
}

void CNPC_WResident::FSM_Idle(_float _fTimeDelta)
{
}

void CNPC_WResident::FSM_Walk(_float _fTimeDelta)
{
    if (m_bPatrolDirection && (m_iActionState == WRESIDENT_A_WALK))
    {
    ...
    if (m_iActionState == WRESIDENT_A_IDLE1)
    {
        ...
        if (m_fCalculateTime1 > 3.f)
        {
            ...
            if (m_bPatrolDirection)
            {
                ...
                m_iActionState = WRESIDENT_A_WALK;
                m_pModel->ChangeAnim(m_iActionState, 0.1f, true);
            }
            else
            {
...
void CNPC_WResident::InteractionOn()
{
}