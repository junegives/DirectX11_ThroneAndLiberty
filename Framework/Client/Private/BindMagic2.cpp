...
HRESULT CBindMagic2::Initialize(BINDMAGIC_DESC* _pBindMagic2Desc, CTransform::TRANSFORM_DESC* _pTransformDesc)
{
    PROJECTILE_DESC* pProjectileDesc = (PROJECTILE_DESC*)_pBindMagic2Desc;

    __super::Initialize(pProjectileDesc, _pTransformDesc);

    // 소유자 지정해주기
    m_pOwner = _pBindMagic2Desc->pOwner;
...
void CBindMagic2::PriorityTick(_float _fTimeDelta)
{
    if (m_bOnlyRender)
        return;
...
void CBindMagic2::Tick(_float _fTimeDelta)
{
    if (m_bStartEffect)
    {
...
void CBindMagic2::LateTick(_float _fTimeDelta)
{
    if (!m_bOnlyRender)
    {
...
HRESULT CBindMagic2::AddRigidBody()
{
    ...
    RectDesc.strShapeTag = "BindMagic2_Body";
    m_strShapeTag = "BindMagic2_Body";
...
void CBindMagic2::OnContactFound(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
    ...
    {
        GAMEINSTANCE->PlaySoundW("MidgetSH_SmallShot", 0.5f);
...
void CBindMagic2::OnContactPersist(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

void CBindMagic2::OnContactLost(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

shared_ptr<CBindMagic2> CBindMagic2::Create(BINDMAGIC_DESC* _pBindMagic2Desc, CTransform::TRANSFORM_DESC* _pTransformDesc)
{
    shared_ptr<CBindMagic2> pInstance = make_shared<CBindMagic2>();

    if (FAILED(pInstance->Initialize(_pBindMagic2Desc, _pTransformDesc)))
        MSG_BOX("Failed to Create : BindMagic2");
