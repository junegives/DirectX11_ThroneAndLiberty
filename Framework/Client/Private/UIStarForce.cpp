...
HRESULT CUIStarForce::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    ...
    m_fLeftEndX = m_fX - 100.f;
    m_fRightEndX = m_fX + 100.f;

    return S_OK;
}

void CUIStarForce::PriorityTick(_float _fTimeDelta)
{

}

void CUIStarForce::Tick(_float _fTimeDelta)
{
    if (m_isLoadingDone)
    {
        KeyInput();
...
void CUIStarForce::LateTick(_float _fTimeDelta)
{
    if (m_isLoadingDone)
    {
        ...
        if (m_bIsRight && !m_bIsPressed)
        {
            m_fTimeAcc += 0.036f;
            if (m_fTimeAcc >= 1.f)
                m_bIsRight = false;
        }
        else if (!m_bIsRight && !m_bIsPressed)
        {
            m_fTimeAcc -= 0.036f;
            if (m_fTimeAcc <= 0.f)
                m_bIsRight = true;
...
void CUIStarForce::SetLeftAndRightEnd(_float _fLeftEndX, _float _fRightEndX)
{
    m_fLeftEndX = _fLeftEndX;
    m_fRightEndX = _fRightEndX;
}

void CUIStarForce::ResetStarForce()
{
    m_isRender = true;
    m_bIsPressed = false;
    m_fTimeAcc = 0.f;
...
HRESULT CUIStarForce::RenderFont()
{
    // 수정 필요
    wstring strButton = L"S T O P !";
    _float fButtonOffset = static_cast<_float>(strButton.size()) * 4.f;

    if (FAILED(GAMEINSTANCE->RenderFont("Font_SuiteL12", strButton, _float2((m_vUIPos[5].x - fButtonOffset) + (g_iWinSizeX * 0.5f), (-m_vUIPos[5].y - 10.f) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_END, _float4(0.79f, 0.65f, 0.28f, 1.f), 0.f, _float2(0.f, 0.f), 1.f)))
        return E_FAIL;

	return S_OK;
}

void CUIStarForce::StarForceKeyInput()
{
    //_int iNumMouse = IsInstanceMouseOn();
}

void CUIStarForce::InitUpdateTexture()
{
...
void CUIStarForce::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    /*if (vertices[4].vTranslation.x <= m_fLeftEndX)
    {
        m_bIsRight = true;
    }

    else if (vertices[4].vTranslation.x >= m_fRightEndX)
    {
        m_bIsRight = false;
    }

    m_fMovePow = LinearIp(1.f, 10.f, m_fTimeAcc);*/

    if (!m_bIsPressed && (!m_bIsFailed && !m_bIsSuccess))
        vertices[3].vTranslation.x = LinearIp(m_fLeftEndX, m_fRightEndX, m_fTimeAcc);

    if (m_bIsFailed || m_bIsSuccess)
    {
        m_fDisappearTime += 0.016f;
        if (m_fDisappearTime >= 1.6f)
        {
            m_bIsFailed = false;
            m_bIsSuccess = false;
    ...
    if (GAMEINSTANCE->MouseDown(DIM_LB) && IsInstanceMouseOn(5) && !m_bIsPressed)
    {
        ...
        if (vertices[3].vTranslation.x >= (vertices[2].vTranslation.x - (vertices[2].vRight.x / 2.f)) &&
            vertices[3].vTranslation.x <= (vertices[2].vTranslation.x + (vertices[2].vRight.x / 2.f)))
        {
            // 일반 성공
            m_bIsFailed = false;
            m_bIsSuccess = true;
                ...
                ITEMMGR->DecreaseItemFromInventory(ITEM_MISC, pItemDesc4.strItemTag, pItemDesc4.numItem))
            {
                ...
                if (!pCraftResult.expired())
                    pCraftResult.lock()->SetSuccessType(0);
            }
        }

        if (m_bIsFailed) // 제작 실패
        {
            weak_ptr<CUICraftTab> pCraftTab = dynamic_pointer_cast<CUICraftTab>(UIMGR->FindUI("CraftFTabs"));
            _uint iTabIndex = pCraftTab.lock()->GetCurTabIndex();
    ...
    /*if (m_bIsRight)
    {
        vertices[4].vTranslation.x += m_fMovePow;
    }
...
HRESULT CUIStarForce::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIStarForce> CUIStarForce::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUIStarForce> pInstance = make_shared<CUIStarForce>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIStarForce::Create", MB_OK);
