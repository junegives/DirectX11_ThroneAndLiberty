...
HRESULT CUIItemHover::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
...
void CUIItemHover::PriorityTick(_float _fDeltaTime)
{

}

void CUIItemHover::Tick(_float _fDeltaTime)
{
    KeyInput();
...
void CUIItemHover::LateTick(_float _fDeltaTime)
{
    ...
    if (!m_pUIChildren.empty())
    {
        for (auto& childPair : m_pUIChildren)
        {
            ...
            {
                if (childPair.second.lock()->GetUIType() == UI_ITEMICON)
                {
...
bool CUIItemHover::IsRender(_float _fVisibleYTop, _float _fVisibleYBottom)
{
...
HRESULT CUIItemHover::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc)
{
...
shared_ptr<CUIItemHover> CUIItemHover::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
    shared_ptr<CUIItemHover> pInstance = make_shared<CUIItemHover>();

    if (FAILED(pInstance->Initialize(_strTextureTags, _strMaskTextureTag, _strNoiseTextureTag, _UIDesc, _InstanceDesc, _initPos)))
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIItemHover::Create", MB_OK);
