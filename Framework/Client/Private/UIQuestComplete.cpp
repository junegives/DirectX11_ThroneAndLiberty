...
HRESULT CUIQuestComplete::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUIQuestComplete::PriorityTick(_float _fTimeDelta)
{

}

void CUIQuestComplete::Tick(_float _fTimeDelta)
{
    if (!m_isLoadingDone)
        return;
...
void CUIQuestComplete::LateTick(_float _fTimeDelta)
{
    if (!m_isLoadingDone)
        return;
...
HRESULT CUIQuestComplete::Render()
{
    ...
    if (!m_strTexKeys.empty())
    {
        ...
        for (_uint i = 0; i < iNumTexKeys; ++i)
        {
            ...
            if (i == 11 && m_pRewardItem)
            {
                if (FAILED(GAMEINSTANCE->BindSRV(shaderKey.c_str(), m_pRewardItem->GetItemTexTag())))
                    return E_FAIL;
            }
...
void CUIQuestComplete::SetQuestReward(const wstring& _strQuestExp, const wstring& _strQuestMoney)
{
    m_strQuestExp = _strQuestExp;
    m_strQuestMoney = _strQuestMoney;
}

void CUIQuestComplete::ResetQuestReward()
{
    m_pRewardItem.reset();
...
HRESULT CUIQuestComplete::RenderFont()
{
    _float strSize = static_cast<_float>(m_strQuestTitle.size()) * 6.5f;

    if (FAILED(GAMEINSTANCE->RenderFont("Font_SuiteL12", m_strQuestTitle, _float2((m_vUIPos[4].x - strSize) + (g_iWinSizeX * 0.5f), (-m_vUIPos[4].y - 120.f) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_END, _float4(0.79f, 0.65f, 0.28f, 1.f))))
        return E_FAIL;
...
void CUIQuestComplete::InitUpdateTexture()
{
    ...
    vertices[4].vColor = _float4(1.f, 1.f, 1.f, 0.3f);

    vertices[6].iSlotUV = 1;
    vertices[8].iSlotUV = 1;

    GAMEINSTANCE->GetDeviceContextInfo()->Unmap(instanceBuffer.Get(), 0);
}

void CUIQuestComplete::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    if (m_pRewardItem)
    {
        UIMGR->ActivateQuestCompleteSlotEffect();
...
HRESULT CUIQuestComplete::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIQuestComplete> CUIQuestComplete::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUIQuestComplete> pInstance = make_shared<CUIQuestComplete>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIQuestComplete::Create", MB_OK);
