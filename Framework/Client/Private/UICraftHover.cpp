...
HRESULT CUICraftHover::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUICraftHover::PriorityTick(_float _fTimeDelta)
{

}

void CUICraftHover::Tick(_float _fTimeDelta)
{
    KeyInput();
...
void CUICraftHover::LateTick(_float _fTimeDelta)
{
...
void CUICraftHover::InitUpdateTexture()
{
...
void CUICraftHover::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
    ...
    for (_uint i = 0; i < iNumSlots; ++i)
    {
        if (i == iNumMouse)
        {
            ...
            {
                weak_ptr<CUICraftTab> pFound = dynamic_pointer_cast<CUICraftTab>(UIMGR->FindUI("CraftFTabs"));
                if (!pFound.expired())
                {
                    if (i == 4)
                        pFound.lock()->SetCurTabIndex(0);
                    else if (i == 2)
                        pFound.lock()->SetCurTabIndex(1);
                    else if (i == 8)
                        pFound.lock()->SetCurTabIndex(2);
                    else if (i == 9)
                        pFound.lock()->SetCurTabIndex(3);
                }
            }
        }
...
HRESULT CUICraftHover::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUICraftHover> CUICraftHover::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUICraftHover> pInstance = make_shared<CUICraftHover>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUICraftHover::Create", MB_OK);
