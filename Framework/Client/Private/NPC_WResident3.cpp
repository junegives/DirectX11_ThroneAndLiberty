...
CNPC_WResident3::CNPC_WResident3()
    : CNPC()
{
}

HRESULT CNPC_WResident3::Initialize(_float3 _vCreatePos)
{
    // 설정된 위치에 생성
    ...
    m_pModel = GAMEINSTANCE->GetModel("WResident3");
    m_strModelName = "WResident3";
    AddComponent(L"Com_Model", m_pModel);
    m_pTransformCom->SetScaling(1.f, 1.f, 1.f);

    switch (m_iGenType)
    {
    case WRESIDENT3_G_IDLE:
        m_iActionState = WRESIDENT3_A_IDLE;
        break;
    case WRESIDENT3_G_WALK:
        m_iActionState = WRESIDENT3_A_IDLE;
        m_pTransformCom->SetSpeed(0.7f);
        break;
    case WRESIDENT3_G_UPSEE:
        m_iActionState = WRESIDENT3_A_UPSEE;
        break;
    case WRESIDENT3_G_SELL1:
        m_iActionState = WRESIDENT3_G_SELL1;
        break;
    case WRESIDENT3_G_SELL2:
        m_iActionState = WRESIDENT3_G_SELL2;
        break;
    }
...
void CNPC_WResident3::PriorityTick(_float _fTimeDelta)
{
    __super::PriorityTick(_fTimeDelta);
}

void CNPC_WResident3::Tick(_float _fTimeDelta)
{
...
void CNPC_WResident3::LateTick(_float _fTimeDelta)
{
...
HRESULT CNPC_WResident3::Render()
{
...
shared_ptr<CNPC_WResident3> CNPC_WResident3::Create(_float3 _vCreatePos, _float3 _vInitLookPos, _int _iGenType, _float3 _vHairColor, _float3 _vHairSubColor)
{
    shared_ptr<CNPC_WResident3> pInstance = make_shared<CNPC_WResident3>();
    ...
    if (FAILED(pInstance->Initialize(_vCreatePos)))
        MSG_BOX("Failed to Create : CNPC_WResident3");

    return pInstance;
}

HRESULT CNPC_WResident3::AddRigidBody(string _strModelKey)
{
...
//UINT			eTargetColFlag = COL_NONE; /* 충돌할 객체의 Flag를 추가 ex) COL_MONSTER | COL_ITEM  */
    ...
    CapsuleDesc.strShapeTag = "NPC_WResident3_Body";
...
void CNPC_WResident3::OnContactFound(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

void CNPC_WResident3::OnContactPersist(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

void CNPC_WResident3::OnContactLost(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

//HRESULT CNPC_WResident3::AddPartObjects(string _strModelKey)
//{
...
void CNPC_WResident3::FSM_Idle(_float _fTimeDelta)
{
}

void CNPC_WResident3::FSM_Walk(_float _fTimeDelta)
{
    if (m_bPatrolDirection && (m_iActionState == WRESIDENT3_A_WALK))
    {
    ...
    if (m_iActionState == WRESIDENT3_A_IDLE)
    {
        ...
        if (m_fCalculateTime1 > 3.f)
        {
            ...
            if (m_bPatrolDirection)
            {
                ...
                m_iActionState = WRESIDENT3_A_WALK;
                m_pModel->ChangeAnim(m_iActionState, 0.1f, true);
            }
            else
            {
...
void CNPC_WResident3::InteractionOn()
{
}