...
CUIInteractionPopUp::CUIInteractionPopUp()
{
...
HRESULT CUIInteractionPopUp::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUIInteractionPopUp::PriorityTick(_float _fTimeDelta)
{

}

void CUIInteractionPopUp::Tick(_float _fTimeDelta)
{
    if (m_isLoadingDone)
    {
        KeyInput();
...
void CUIInteractionPopUp::LateTick(_float _fTimeDelta)
{
    if (m_isLoadingDone)
    {
...
HRESULT CUIInteractionPopUp::Render()
{
...
void CUIInteractionPopUp::RenderScript()
{
...
HRESULT CUIInteractionPopUp::RenderFont()
{
    //wstring strDisplayContent = L"팝업 테스트";
    _float fContentOffset = static_cast<_float>(m_strDisplayContent.size()) * 6.f;
    _float fContentYOffset = 20.f;

    if (m_strDisplayContent == L"나를 지키는 수호자들이 각자의 고통을 마주하여\n광명을 되찾을때 나도 비로소 광명을 얻으리라")
    {
        fContentOffset = 160.f;
        fContentYOffset = 20.f;
    }

    if (m_strDisplayContent == L"그날의 추위는 온 주위를 뒤덮었고 그는 그의 숙명을\n계속 이어갈수 없었다")
    {
        fContentOffset = 165.f;
        fContentYOffset = 20.f;
    }

    if (m_strDisplayContent == L"밤은 그의 시간이나 다른 시간은 그렇지 못했다")
    {
        fContentOffset = 155.f;
        fContentYOffset = 10.f;
    }

    if (m_strDisplayContent == L"그녀는 보답없는 자신의 계속된 헌신에 지쳤고\n결국 마음이 얼어버렸다")
    {
        fContentOffset = 145.f;
        fContentYOffset = 20.f;
    }

    if (m_strDisplayContent == L"앞이 보이지 않는 그에게 온몸에서 느껴지는 작열감은\n끔찍했다")
    {
        fContentOffset = 170.f;
        fContentYOffset = 20.f;
    }

    if (FAILED(GAMEINSTANCE->RenderFont("Font_DNFB12", m_strDisplayContent, _float2((m_vUIPos[0].x - fContentOffset) + (g_iWinSizeX * 0.5f), (-m_vUIPos[0].y - fContentYOffset) + (g_iWinSizeY * 0.5f)), CCustomFont::FA_END, _float4(0.79f, 0.65f, 0.28f, 1.f), 0.f, _float2(0.f, 0.f), 1.f)))
        return E_FAIL;

    return S_OK;
}

void CUIInteractionPopUp::InitUpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
...
void CUIInteractionPopUp::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
...
HRESULT CUIInteractionPopUp::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIInteractionPopUp> CUIInteractionPopUp::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUIInteractionPopUp> pInstance = make_shared<CUIInteractionPopUp>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIInteractionPopUp::Create", MB_OK);
