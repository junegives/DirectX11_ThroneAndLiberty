...
void CParticleSystem::Tick(_float _fTimeDelta)
{
    ...
    {
        // 전체 재생 끝났을 때
        if ((m_eParticleSystem.fDuration <= m_fActiveTime) && !m_eParticleSystem.bLoop)
            m_bCanEmit = false;

        else if (1 == m_eParticleSystem.iPosType || 2 == m_eParticleSystem.iPosType)
        {
            if (!CheckOwnerEnable())
                m_bCanEmit = false;
        }
...
HRESULT CParticleSystem::Initialize_Desc(PARTICLE_SYSTEM* pDesc)
{
    ...
    // Pos
    {
        ...
        m_eParticleSystem.bRelativeOwner = pDesc->bRelativeOwner;
    }
...
void CParticleSystem::UpdateParticles(_float _fTimeDelta)
{
    ...
    for (_int i = 0; i < m_eParticleSystem.iMaxParticles; i++)
    {
        ...
        if (!m_eParticleSystem.bSpeedMode)
        {
			...
			_int iKeyFramesCnt = (int)m_eParticleSystem.KeyFrames.SpeedKeyFrames.size();
		...
		_int iKeyFramesCnt = (int)m_eParticleSystem.KeyFrames.ScaleKeyFrames.size();
        ...
        if (m_eParticleSystem.bUseColor)
        {
            ...
            _int iKeyFramesCnt = (int)m_eParticleSystem.KeyFrames.ColorKeyFrames.size();
...
void CParticleSystem::CreateParticle(_int _iIndex)
{
    ...
    else
    {
...

        {
            ...
            _float3 parentWorldZAxis = -SimpleMath::Vector3::TransformNormal(SimpleMath::Vector3::UnitZ, m_OwnerWorld);

            // 자식의 로컬 포지션 벡터를 부모의 월드 행렬의 Look 벡터에 대한 회전 행렬로 변환합니다.
            _float3 rotatedLocalPosition = SimpleMath::Vector3::Transform(m_vCurPos, SimpleMath::Matrix::CreateWorld(SimpleMath::Vector3::Zero, parentWorldZAxis, SimpleMath::Vector3::UnitY));

            // 부모의 월드 행렬의 위치 벡터에 회전된 자식의 로컬 포지션 벡터를 더하여 자식의 월드 행렬의 위치를 계산합니다.
            _float3 childWorldPosition = m_OwnerWorld.Translation() + rotatedLocalPosition;

            // 자식의 월드 행렬을 구성합니다.
            m_vCurPos = SimpleMath::Matrix::CreateTranslation(childWorldPosition).Translation();
        }
...
_bool CParticleSystem::CheckOwnerEnable()
{
    if (!m_pOwner.lock() || !m_pOwner.lock()->IsEnabled())
    {
        return false;
