...
CNPC_Resident1::CNPC_Resident1()
    : CNPC()
{
}

HRESULT CNPC_Resident1::Initialize(_float3 _vCreatePos)
{
    // 설정된 위치에 생성
    ...
    m_pModel = GAMEINSTANCE->GetModel("Resident1");
    m_strModelName = "Resident1";
    AddComponent(L"Com_Model", m_pModel);
    m_pTransformCom->SetScaling(1.f, 1.f, 1.f);

    switch (m_iGenType)
    {
    case RESIDENT1_G_DRUNKEN:
        m_iActionState = RESIDENT1_A_DRUNKEN;
        break;
    case RESIDENT1_G_ANGRY:
        m_iActionState = RESIDENT1_A_ANGRY;
        m_pTransformCom->SetSpeed(0.7f);
        break;
    case RESIDENT1_G_IDLE:
        m_iActionState = RESIDENT1_A_IDLE;
        break;
    case RESIDENT1_G_TALK:
        m_iActionState = RESIDENT1_A_TALK;
        break;
    case RESIDENT1_G_WALK:
        m_iActionState = RESIDENT1_A_WALK;
        break;
    }
...
void CNPC_Resident1::PriorityTick(_float _fTimeDelta)
{
    __super::PriorityTick(_fTimeDelta);
}

void CNPC_Resident1::Tick(_float _fTimeDelta)
{
...
void CNPC_Resident1::LateTick(_float _fTimeDelta)
{
...
shared_ptr<CNPC_Resident1> CNPC_Resident1::Create(_float3 _vCreatePos, _float3 _vInitLookPos, _int _iGenType, _float3 _vHairColor, _float3 _vHairSubColor)
{
    shared_ptr<CNPC_Resident1> pInstance = make_shared<CNPC_Resident1>();
    ...
    if (FAILED(pInstance->Initialize(_vCreatePos)))
        MSG_BOX("Failed to Create : CNPC_Resident1");

    return pInstance;
}

HRESULT CNPC_Resident1::AddRigidBody(string _strModelKey)
{
...
//UINT			eTargetColFlag = COL_NONE; /* 충돌할 객체의 Flag를 추가 ex) COL_MONSTER | COL_ITEM  */
    ...
    CapsuleDesc.strShapeTag = "NPC_Resident1_Body";
...
void CNPC_Resident1::OnContactFound(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

void CNPC_Resident1::OnContactPersist(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

void CNPC_Resident1::OnContactLost(const COLLISIONDATA& _ColData, const string& _szMyShapeTag)
{
}

//HRESULT CNPC_Resident1::AddPartObjects(string _strModelKey)
//{
...
void CNPC_Resident1::FSM_Idle(_float _fTimeDelta)
{
}

void CNPC_Resident1::FSM_Walk(_float _fTimeDelta)
{
    if (m_bPatrolDirection && (m_iActionState == RESIDENT1_A_WALK))
    {
    ...
    if (m_iActionState == RESIDENT1_A_IDLE)
    {
...
void CNPC_Resident1::InteractionOn()
{
}