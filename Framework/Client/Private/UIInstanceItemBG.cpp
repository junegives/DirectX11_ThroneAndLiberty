...
CUIInstanceItemBG::CUIInstanceItemBG()
{
...
HRESULT CUIInstanceItemBG::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUIInstanceItemBG::PriorityTick(_float _fTimeDelta)
{

}

void CUIInstanceItemBG::Tick(_float _fTimeDelta)
{
    KeyInput();
...
void CUIInstanceItemBG::LateTick(_float _fTimeDelta)
{
    ...
    if (!m_pUIChildren.empty())
    {
        for (auto& childPair : m_pUIChildren)
        {
            if (childPair.second.lock() != nullptr)
            {
                if (childPair.second.lock()->GetUIType() == UI_INSTANCEITEMSLOT)
                {
                    if (childPair.second.lock()->GetUITag() == "InvEquipSlot0")
                        dynamic_pointer_cast<CUIInstanceItemSlot>(childPair.second.lock())->IsRender(m_fVisibleTop, m_fVisibleBottom);
...
HRESULT CUIInstanceItemBG::Render()
{
...
void CUIInstanceItemBG::IsRender(_float _fVisibleYTop, _float _fVisibleYBottom)
{
...
HRESULT CUIInstanceItemBG::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUIInstanceItemBG> CUIInstanceItemBG::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUIInstanceItemBG> pInstance = make_shared<CUIInstanceItemBG>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUIInstanceItemBG::Create", MB_OK);
