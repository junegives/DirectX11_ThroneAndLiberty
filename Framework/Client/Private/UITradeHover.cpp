...
HRESULT CUITradeHover::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
void CUITradeHover::PriorityTick(_float _fTimeDelta)
{

}

void CUITradeHover::Tick(_float _fTimeDelta)
{
    if (!m_isLoadingDone)
        return;
...
void CUITradeHover::LateTick(_float _fTimeDelta)
{
    if (!m_isLoadingDone)
        return;
...
HRESULT CUITradeHover::RenderFont()
{
...
void CUITradeHover::TradeKeyInput()
{
    _int iNumMouse = IsInstanceMouseOn();

    if (GAMEINSTANCE->MouseDown(DIM_LB) && iNumMouse != -1 && iNumMouse == 35)
    {
        shared_ptr<CUITradeSlot> pUI = dynamic_pointer_cast<CUITradeSlot>(UIMGR->FindUI("TradeCSlots"));
        if (pUI == nullptr)
            return;

        if (!pUI->DecreaseNumItem())
            return;
    }

    if (GAMEINSTANCE->MouseDown(DIM_LB) && iNumMouse != -1 && iNumMouse == 36)
    {
        shared_ptr<CUITradeSlot> pUI = dynamic_pointer_cast<CUITradeSlot>(UIMGR->FindUI("TradeCSlots"));
        if (pUI == nullptr)
            return;

        if (!pUI->IncreaseNumItem())
            return;
    }
}

void CUITradeHover::InitUpdateTexture()
{
...
void CUITradeHover::UpdateTexture()
{
    D3D11_MAPPED_SUBRESOURCE subResource{};
...
HRESULT CUITradeHover::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CUITradeHover> CUITradeHover::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CUIBase::UIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CUITradeHover> pInstance = make_shared<CUITradeHover>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CUITradeHover::Create", MB_OK);
