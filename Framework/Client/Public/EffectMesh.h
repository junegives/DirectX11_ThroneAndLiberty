        ...
        _float4*    GetCurColor(_float _fTime, _int iSplitCnt)
        {
            ...
            for (auto iter = ColorKeyFrames.begin(); iter != ColorKeyFrames.end(); ++iter)
            {
                ...
                if (iter + 1 != ColorKeyFrames.end())
                {
                    ...
                    if ((iter + 1)->fTime <= _fTime)
                        continue;

                    for (_int i = 0; i < iSplitCnt; ++i)
                    {
                        vCurColors[i].x = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, (iter + 1)->fTime - iter->fTime, iter->vColor[i].x, (iter + 1)->vColor[i].x);
                        vCurColors[i].y = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, (iter + 1)->fTime - iter->fTime, iter->vColor[i].y, (iter + 1)->vColor[i].y);
                        vCurColors[i].z = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, (iter + 1)->fTime - iter->fTime, iter->vColor[i].z, (iter + 1)->vColor[i].z);
                        vCurColors[i].w = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, (iter + 1)->fTime - iter->fTime, iter->vColor[i].w, (iter + 1)->vColor[i].w);
                    }
        ...
        _float4		GetCurGlowColor(_float _fTime)
        {
            ...
            for (auto iter = GlowColorKeyFrames.begin(); iter != GlowColorKeyFrames.end(); ++iter)
            {
                ...
                if (iter + 1 != GlowColorKeyFrames.end())
                {
                    ...
                    vCurColor.x = GAMEINSTANCE->Ease(EASE_LINEAR, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vGlowColor.x, iterNext->vGlowColor.x);
                    vCurColor.y = GAMEINSTANCE->Ease(EASE_LINEAR, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vGlowColor.y, iterNext->vGlowColor.y);
                    vCurColor.z = GAMEINSTANCE->Ease(EASE_LINEAR, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vGlowColor.z, iterNext->vGlowColor.z);
                    vCurColor.w = GAMEINSTANCE->Ease(EASE_LINEAR, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vGlowColor.w, iterNext->vGlowColor.w);
        ...
        _float3		GetCurScale(_float _fTime)
        {
            ...
            for (auto iter = ScaleKeyFrames.begin(); iter != ScaleKeyFrames.end(); ++iter)
            {
                ...
                if (iter + 1 != ScaleKeyFrames.end())
                {
                    ...
                    vCurScale.x = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vScale.x, iterNext->vScale.x);
                    vCurScale.y = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vScale.y, iterNext->vScale.y);
                    vCurScale.z = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vScale.z, iterNext->vScale.z);
        ...
        _float3		GetCurTrans(_float _fTime)
        {
            ...
            for (auto iter = TransKeyFrames.begin(); iter != TransKeyFrames.end(); ++iter)
            {
                ...
                if (iter + 1 != TransKeyFrames.end())
                {
                    ...
                    vCurTrans.x = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vTranslation.x, iterNext->vTranslation.x);
                    vCurTrans.y = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vTranslation.y, iterNext->vTranslation.y);
                    vCurTrans.z = GAMEINSTANCE->Ease((EASE_TYPE)iter->eEaseType, _fTime - iter->fTime, iterNext->fTime - iter->fTime, iter->vTranslation.z, iterNext->vTranslation.z);
...
private:
    ...
    _float3                 m_vCurRotX = _float3(0.f, 0.f, 0.f);
    _float3                 m_vCurRotY = _float3(0.f, 0.f, 0.f);
    _float3                 m_vCurRotZ = _float3(0.f, 0.f, 0.f);
