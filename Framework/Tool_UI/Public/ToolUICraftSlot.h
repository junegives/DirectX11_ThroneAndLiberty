...
class CToolUICraftSlot : public CToolUIBase
{
	using Super = CToolUIBase;

public:
	CToolUICraftSlot();
	virtual ~CToolUICraftSlot();
...
public:
	static shared_ptr<CToolUICraftSlot> Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize);
