...
class CToolUICraftResult : public CToolUIBase
{
	using Super = CToolUIBase;

public:
	CToolUICraftResult();
	virtual ~CToolUICraftResult();
...
public:
	static shared_ptr<CToolUICraftResult> Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize);
