...
HRESULT CToolUIBar::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc)
{
...
void CToolUIBar::PriorityTick(_float _fDeltaTime)
{
}

void CToolUIBar::Tick(_float _fDeltaTime)
{
    KeyInput();
...
void CToolUIBar::LateTick(_float _fDeltaTime)
{
...
HRESULT CToolUIBar::Render()
{
    ...
    if (FAILED(GAMEINSTANCE->BindRawValue("g_fRatio", &m_fRatio, sizeof(_float))))
        return E_FAIL;
...
HRESULT CToolUIBar::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc)
{
    ...
    /*_uint tagSize = static_cast<_uint>(_strTextureTags.size());
    for (_uint i = 0; i < tagSize; ++i)
    {
        shared_ptr<CTexture> pTexture = GAMEINSTANCE->GetTexture(_strTextureTags[i]);
        if (pTexture == nullptr)
            return E_FAIL;
    ...
    if (m_ToolUIDesc.m_isMasked)
    {
        /*shared_ptr<CTexture> pTexture = GAMEINSTANCE->GetTexture(_strMaskTextureTag);
        if (pTexture == nullptr)
            return E_FAIL;

        m_pMaskTexture = pTexture;*/
...
shared_ptr<CToolUIBar> CToolUIBar::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc)
{
    shared_ptr<CToolUIBar> pInstance = make_shared<CToolUIBar>();

    if (FAILED(pInstance->Initialize(_strTextureTags, _strMaskTextureTag, _strNoiseTextureTag, _ToolUIDesc, _InstanceDesc)))
    {
        MessageBox(nullptr, L"Initialize Failed", L"CToolUIBar::Create", MB_OK);
