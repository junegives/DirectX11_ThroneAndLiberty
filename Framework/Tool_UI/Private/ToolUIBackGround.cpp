...
CToolUIBackGround::CToolUIBackGround()
{
...
HRESULT CToolUIBackGround::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc)
{
    ...
    if (m_ToolUIDesc.m_eUIType == TOOL_UI_BACKGROUND_R)
    {
        m_iShaderPass = 9;
        m_ToolUIDesc.iShaderPassIdx = 9;
    }

    else if (m_ToolUIDesc.m_eUIType == TOOL_UI_BACKGROUND_NT)
    {
        m_iShaderPass = 11;
        m_ToolUIDesc.iShaderPassIdx = 11;
    }

    else if (m_ToolUIDesc.m_eUIType == TOOL_UI_HL)
    {
        m_isRight = 0;
        m_iShaderPass = 13;
        m_ToolUIDesc.iShaderPassIdx = 13;
    }

    else if (m_ToolUIDesc.m_eUIType == TOOL_UI_HR)
    {
        m_isRight = 1;
        m_iShaderPass = 13;
        m_ToolUIDesc.iShaderPassIdx = 13;
...
void CToolUIBackGround::PriorityTick(_float _fDeltaTime)
{

}

void CToolUIBackGround::Tick(_float _fDeltaTime)
{
    KeyInput();
    ...
    if (!m_pUIChildren.empty())
    {
        for (auto& childPair : m_pUIChildren)
        {
            ...
            if (childPair.second.lock()->GetUIType() == TOOL_UI_ITEMTAB)
            {
                weak_ptr<CToolUIItemTab> childTab = dynamic_pointer_cast<CToolUIItemTab>(childPair.second.lock());
                if (!childTab.expired())
                {
...
void CToolUIBackGround::LateTick(_float _fDeltaTime)
{
...
HRESULT CToolUIBackGround::Render()
{
...
HRESULT CToolUIBackGround::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc)
{
...
shared_ptr<CToolUIBackGround> CToolUIBackGround::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc)
{
    shared_ptr<CToolUIBackGround> pInstance = make_shared<CToolUIBackGround>();

    if (FAILED(pInstance->Initialize(_strTextureTags, _strMaskTextureTag, _strNoiseTextureTag, _ToolUIDesc, _InstanceDesc)))
    {
        MessageBox(nullptr, L"Initialize Failed", L"CToolUIBackGround::Create", MB_OK);
