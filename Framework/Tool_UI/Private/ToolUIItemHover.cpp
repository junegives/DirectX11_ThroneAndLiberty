...
CToolUIItemHover::CToolUIItemHover()
{
...
HRESULT CToolUIItemHover::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
...
void CToolUIItemHover::PriorityTick(_float _fDeltaTime)
{

}

void CToolUIItemHover::Tick(_float _fDeltaTime)
{
    KeyInput();
...
void CToolUIItemHover::LateTick(_float _fDeltaTime)
{
    if (m_isHovered)
    {
    ...
    if (!m_pUIChildren.empty())
    {
        for (auto& childPair : m_pUIChildren)
        {
            if (!childPair.second.expired())
            {
                if (childPair.second.lock()->GetUIType() == TOOL_UI_ITEMICON)
                {
...
HRESULT CToolUIItemHover::Render()
{
...
bool CToolUIItemHover::IsRender(_float _fVisibleYTop, _float _fVisibleYBottom)
{
...
HRESULT CToolUIItemHover::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc)
{
...
shared_ptr<CToolUIItemHover> CToolUIItemHover::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
    shared_ptr<CToolUIItemHover> pInstance = make_shared<CToolUIItemHover>();

    if (FAILED(pInstance->Initialize(_strTextureTags, _strMaskTextureTag, _strNoiseTextureTag, _ToolUIDesc, _InstanceDesc, _initPos)))
    {
        MessageBox(nullptr, L"Initialize Failed", L"CToolUIItemHover::Create", MB_OK);
