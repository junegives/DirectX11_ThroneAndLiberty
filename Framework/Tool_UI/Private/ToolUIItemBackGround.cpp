...
CToolUIItemBackGround::CToolUIItemBackGround()
{
...
HRESULT CToolUIItemBackGround::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
...
void CToolUIItemBackGround::PriorityTick(_float _fDeltaTime)
{

}

void CToolUIItemBackGround::Tick(_float _fDeltaTime)
{
    KeyInput();
...
void CToolUIItemBackGround::LateTick(_float _fDeltaTime)
{
    ...
    if (!m_pUIChildren.empty())
    {
        for (auto& childPair : m_pUIChildren)
        {
            if (childPair.second.lock() != nullptr)
            {
                ...
                {
                    if (dynamic_pointer_cast<CToolUISlot>(childPair.second.lock())->IsRender(m_fVisibleTop, m_fVisibleBottom))
...
HRESULT CToolUIItemBackGround::Render()
{
...
bool CToolUIItemBackGround::IsRender(_float _fVisibleYTop, _float _fVisibleYBottom)
{
...
HRESULT CToolUIItemBackGround::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc)
{
...
shared_ptr<CToolUIItemBackGround> CToolUIItemBackGround::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
    shared_ptr<CToolUIItemBackGround> pInstance = make_shared<CToolUIItemBackGround>();

    if (FAILED(pInstance->Initialize(_strTextureTags, _strMaskTextureTag, _strNoiseTextureTag, _ToolUIDesc, _InstanceDesc, _initPos)))
    {
        MessageBox(nullptr, L"Initialize Failed", L"CToolUIItemBackGround::Create", MB_OK);
