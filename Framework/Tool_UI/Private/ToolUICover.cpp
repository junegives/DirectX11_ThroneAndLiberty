...
HRESULT CToolUICover::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
    ...
    if (m_ToolUIDesc.m_eUIType == TOOL_UI_COVER_RV) /* Reverse Vertically */
    {
        m_iShaderPass = 12;
        m_ToolUIDesc.iShaderPassIdx = 12;
    }

    return S_OK;
}

void CToolUICover::PriorityTick(_float _fDeltaTime)
{

}

void CToolUICover::Tick(_float _fDeltaTime)
{
    KeyInput();
...
void CToolUICover::LateTick(_float _fDeltaTime)
{
...
HRESULT CToolUICover::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc)
{
...
shared_ptr<CToolUICover> CToolUICover::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _ToolUIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, _float2 _initPos)
{
    shared_ptr<CToolUICover> pInstance = make_shared<CToolUICover>();

    if (FAILED(pInstance->Initialize(_strTextureTags, _strMaskTextureTag, _strNoiseTextureTag, _ToolUIDesc, _InstanceDesc, _initPos)))
    {
        MessageBox(nullptr, L"Initialize Failed", L"CToolUICover::Create", MB_OK);
