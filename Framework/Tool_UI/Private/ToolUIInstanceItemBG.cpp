...
CToolUIInstanceItemBG::CToolUIInstanceItemBG()
{
}
...
HRESULT CToolUIInstanceItemBG::Initialize(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    ...
    //m_InstanceDesc.fSizeY = _InstanceDesc.vSize.y;
...
void CToolUIInstanceItemBG::PriorityTick(_float _fTimeDelta)
{

}

void CToolUIInstanceItemBG::Tick(_float _fTimeDelta)
{
    KeyInput();

    _int iMouseOnSlot = IsInstanceMouseOn();
    ...
    if (iMouseOnSlot != -1)
    {
        ...
        vertices[iMouseOnSlot].iTexIndex = 1;
    }
...
void CToolUIInstanceItemBG::LateTick(_float _fTimeDelta)
{
    ...
    if (!m_pUIChildren.empty())
    {
        for (auto& childPair : m_pUIChildren)
        {
            if (childPair.second.lock() != nullptr)
            {
                if (childPair.second.lock()->GetUIType() == TOOL_UI_INSTANCEITEMSLOT)
                {
                    dynamic_pointer_cast<CToolUIInstanceItemSlot>(childPair.second.lock())->IsRender(m_fVisibleTop, m_fVisibleBottom);
...
HRESULT CToolUIInstanceItemBG::Render()
{
...
void CToolUIInstanceItemBG::IsRender(_float _fVisibleYTop, _float _fVisibleYBottom)
{
...
_int CToolUIInstanceItemBG::IsInstanceMouseOn()
{
    POINT mousePt;
...
HRESULT CToolUIInstanceItemBG::AddComponent(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
...
shared_ptr<CToolUIInstanceItemBG> CToolUIInstanceItemBG::Create(vector<string>& _strTextureTags, const string& _strMaskTextureTag, const string& _strNoiseTextureTag, CToolUIBase::ToolUIDesc& _UIDesc, CVIInstancing::InstanceDesc& _InstanceDesc, vector<_float2>& _vUIPos, vector<_float2>& _vUISize)
{
    shared_ptr<CToolUIInstanceItemBG> pInstance = make_shared<CToolUIInstanceItemBG>();
    ...
    {
        MessageBox(nullptr, L"Initialize Failed", L"CToolUIInstanceItemBG::Create", MB_OK);
